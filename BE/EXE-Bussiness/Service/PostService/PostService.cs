...
namespace EXE_Bussiness.Service.PostService
{
    public class PostService : IPostService
    {
        ...
        public PostService(AppDBContext context, IMapper mapper, ILogger<PostService> logger)
        {
            _mapper = mapper;
            _context = context;
            _logger = logger;
        ...
        {
            var posts = _context.Posts.Include(x => x.User).Include(x => x.SubCategory).AsQueryable();
        ...
        {
            var post = await _context.Posts.Include(x => x.User).Include(x => x.SubCategory).FirstOrDefaultAsync(x => x.Id == id);
            if(post == null)
            {
                _logger.LogError("Post not found");
                return null;
        ...
        {
            var post = await _context.Posts.Include(x => x.User).Include(x => x.SubCategory).ToListAsync();
